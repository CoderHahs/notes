...
One of the goals of statistical inference to draw conclusions about a _population_ based on a random sample from the population.
...
The point estimate is obtained using a _statistic_, which is simply a function of a random sample. The probability distribution of the statistic is its **sampling distribution**.
...
if the variance of the original population is _unknown_, then it is estimated by the sample variance $S^2$ and the estimated standard error $\overline{X}$:
...
If σ is unknown, it can be shown that $\frac{X-\mu}{S/\sqrt{n}}$
...
$100(1 - α)%$ C.I. is $X ± z_{α/2}\frac{S}{\sqrt{n}}$.
