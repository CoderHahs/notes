...
The (random) quantity we use to measure evidence is called a _test statistic_. We need to know its sampling distribution when $H_0$ is true.

The _$p$-value_ quantifies "the evidence against $H_0$". A smaller $p$-value means more evidence against claim.
...
- a _null hypothesis_, denoted by $H_0$
- an _alternative hypothesis_, denoted by $H_1$ or $H_A$

The hypothesis is tested by evaluating experimental evidence:

- we _reject_ $H_0$ if the evidence against $H_0$ is _strong_
- we _fail to reject_ $H_0$ if the evidence against is $H_0$ is _insufficient_
...
We say that $Z_0$ is the observed value of the **Z-test statistic** $Z_0$. If z_0 < 0, we have evidence that $\mu$ < $\mu_0$. However, we only reject $H_0$ in favour of $H_1$ if the evidence is _significant_.
...
**Decision Rule**: if the p-value $\leq \alpha$, then we _reject_ $H_0$ in favour of $H_1$. If the p-value $> \alpha$, we _fail to reject_ $H_0$.
